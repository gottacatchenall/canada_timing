...
From this, we can get our cumalitive distribution function (cdf), $\Pi(t)$, which gives us the probability that a sample from $T$ is less than $t$. The cdf $\Pi(t)$ is defined as $$\Pi(T) = \int_{-\infty}^T \pi(t) dt$$
...
data.frame(min_alpha = c(0.8,3,5)) %>%
  ...
  mutate(max_facet = paste('$\\alpha_{max} = ', max_alpha ,'$', sep='')) %>%
    mutate(max_facet = factor(max_facet, levels=c("$\\alpha_{max} = 5$","$\\alpha_{max} = 7$","$\\alpha_{max} = 10$"))) %>%
